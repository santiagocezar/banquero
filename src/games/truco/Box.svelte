...
{#snippet match(i, x, y, rot)}
    <g
        class="match"
        data-ghost={i > minCount && i <= maxCount}
        data-hidden={i > maxCount}
    >
        <rect
            class="stick"
            transform="translate({x}, {y}) rotate({rot})"
            x={-MATCH_STICK_WIDTH / 2}
            y={-MATCH_LENGTH / 2 + MATCH_HEAD_LENGTH / 2}
            width={MATCH_STICK_WIDTH}
            height={MATCH_STICK_LENGTH + MATCH_HEAD_LENGTH / 2}
            ry={MATCH_STICK_CORNER}
        />
        <rect
            class="head"
            transform="translate({x}, {y}) rotate({rot})"
            x={-MATCH_HEAD_WIDTH / 2}
            y={-MATCH_LENGTH / 2}
            width={MATCH_HEAD_WIDTH}
            height={MATCH_HEAD_LENGTH}
            ry={MATCH_HEAD_CORNER}
        />
    </g>
{/snippet}
...
<style lang="less">
    ...
    .match {
        transition:
            0.2s opacity linear,
            0.3s filter cubic-bezier(0.17, 0.67, 0.12, 1),
            0.3s transform cubic-bezier(0.17, 0.67, 0.12, 1);
